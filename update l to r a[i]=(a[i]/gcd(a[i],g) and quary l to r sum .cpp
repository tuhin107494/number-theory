...
void  update (int u,int b,int e,int i,int j,int val)
{
    ...
    int mid=(b+e)/2;
    update(2*u,b,mid,i,j,val);
    update((2*u)+1,mid+1,e,i,j,val);
    tr[u]=tr[2*u]+tr[2*u +1];
}
        ...
        cin>>l>>r>>g;
         ...
         while(g!=1)
         {
            ...
            vector<ll>delet;
              ...
              for(auto it=v[d].lower_bound(l);it!=v[d].end() and *it<=r;it++)
              {
                if(*it<l)continue;
                ll id=*it;
                ...
                if(a[id]%d!=0)
                {
                    ...
                    //cout<<d<<" "<<id<<" "<<a[id]<<endl;
                    update(1,1,n,id,id,a[id]);
                
            }

            for(auto id:delet)v[d].erase(id);
